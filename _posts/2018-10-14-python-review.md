...
## 11. 리스트
여러 데이터를 하나로 묶어 저장 및 관리하는 기능이다. 대괄호([]) 안에 데이터 여러 개를 쉼표(,)로 구분하여 나열한다. 리스트의 자료 값의 위치 번호는 1이 아닌 0부터 시작한다.
```Python
a = [1, 3, 5, 8, 13]
```
```
...
| 함수 | 동작 | 예시 |
|:---|:---:|:---:|
|len(a)|리스트의 길이|a = [1, 2, 3, 4, 5] <br> >>> len(a) <br> 5|
|append(x)|데이터 x를 리스트의 맨 뒤에 추가|a = [1,2,3,4,5] <br> >>> a.append(6) <br> a =  [1, 2, 3, 4, 5, 6]|
|insert(i, x)|리스트의 i번에 x 추가|a = [1, 2] <br> >>> a.insert(0, 1) <br> a = 0, 1, 2|
|pop(i)|리스트의 i번에 있는 데이터를 빼서 리턴, i를 생략하면 맨 마지막 값 리턴|a = [1, 2, 3] <br> >>> a.pop() <br> 3|
|clear()|리스트의 모든 데이터를 삭제|a = [1, 2, 3] <br> a.clear()|
|x in a|리스트 a 안에 x라는 데이터가 있는지 판단|a = [1, 2, 3] <br> >>> 3 in a <br> True <br> >>> 5 in a <br> False|
